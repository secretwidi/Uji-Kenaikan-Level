		...
		if ($this->session->userdata('username')) {
			if ($this->session->userdata('level_name') == 'superadmin') {
				if ($this->admin_model->add_admin() == TRUE) {
					$this->session->set_flashdata('success', 'Tambah admin sukses');
					redirect('admin');
				}else{
		...
		if ($this->session->userdata('username')) {
			if ($this->session->userdata('level_name') == 'superadmin') {
				if ($this->admin_model->edit_admin() == TRUE) {
					$this->session->set_flashdata('success', 'Edit admin sukses');
					redirect('admin');
				}else{
					$this->session->set_flashdata('failed', 'Edit admin sukses');
					redirect('admin');
				}
			}else {
				redirect('dashboard');
