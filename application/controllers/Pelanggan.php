	...
	public function proses_edit()
	{
		if ($this->session->userdata('username')) {
			if ($this->pelanggan_model->edit_pelanggan() == TRUE) {
				$this->session->set_flashdata('success', 'Edit pelanggan sukses');
				redirect('pelanggan/list_pelanggan');
			}else{
				$this->session->set_flashdata('failed', 'Edit pelanggan gagal');
	...
	public function add_pelanggan()
	{
		if ($this->session->userdata('username')) {
			if ($this->pelanggan_model->add_pelanggan() == TRUE) {
				$this->session->set_flashdata('success', 'Tambah pelanggan sukses');
				redirect('pelanggan/list_pelanggan');
			}else{
				$this->session->set_flashdata('failed', 'Tambah pelanggan gagal');
				redirect('pelanggan/list_pelanggan');
			}
		}else{
			redirect('login');
	...
	public function delete_pelanggan($id)
	{
		if ($this->session->userdata('username')) {
			if ($this->pelanggan_model->delete_pelanggan($id) == TRUE) {
				$this->session->set_flashdata('success', 'Hapus pelanggan sukses');
				redirect('pelanggan/list_pelanggan');
			}else{
				$this->session->set_flashdata('failed', 'Hapus pelanggan gagal');
				redirect('pelanggan/list_pelanggan');
			}
		}else{
			redirect('login');
