		...
		if ($this->session->userdata('username') || $this->session->userdata('username_pelanggan')) {
			...
			if ($id_order != 0) {
				...
				if ($this->transaksi_model->add_order_detail($detail) == TRUE) {
					$this->session->set_flashdata('success', 'Transaksi sukses');
					if ($this->session->userdata('username_pelanggan')) {
						redirect('pelanggan');
					}else{
						redirect('transaksi');
	...
	public function delete_transaksi($id)
	{
		if ($this->session->userdata('username')) {
			if ($this->transaksi_model->delete_transaksi($id) == TRUE) {
				$this->session->set_flashdata('success', 'Hapus transaksi sukses');
				redirect('transaksi');
			}else{
				$this->session->set_flashdata('failed', 'Hapus transaksi gagal');
				redirect('transaksi');
			}
		}else{
			redirect('login');
